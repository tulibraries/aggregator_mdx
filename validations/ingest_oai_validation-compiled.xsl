...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!--SCHEMATRON PATTERNS-->
   <!--PATTERN RequiredElementsPatternTitle field required-->
   <!--RULE -->
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:message xmlns:iso="http://purl.oclc.org/dsdl/schematron">
               ...
               <xsl:text>There must be a title</xsl:text>
            </xsl:message>
         </xsl:otherwise>
      </xsl:choose>
   ...
   <!--PATTERN NoHarvestNo stopword required-->
   <!--RULE -->
   <xsl:template match="//*" priority="1000" mode="M8">

		<!--ASSERT error-->
      <xsl:choose>
         <xsl:when test="not(contains(normalize-space(.),'pdcp_noharvest'))"/>
         <xsl:otherwise>
            <xsl:message xmlns:iso="http://purl.oclc.org/dsdl/schematron">
               <xsl:text>Error:</xsl:text>
               <xsl:text>The record must not contain stopword</xsl:text>
            </xsl:message>
         </xsl:otherwise>
      </xsl:choose>
   ...
   <!--PATTERN CollectionCollection record check-->
   <!--RULE -->
   <xsl:template match="//*[local-name()='type']" priority="1000" mode="M9">

		<!--ASSERT error-->
      <xsl:choose>
         <xsl:when test="not(starts-with(normalize-space(.),'Collection'))"/>
         <xsl:otherwise>
            <xsl:message xmlns:iso="http://purl.oclc.org/dsdl/schematron">
               <xsl:text>Error:</xsl:text>
               <xsl:text>Collection records are invalid</xsl:text>
            </xsl:message>
         </xsl:otherwise>
      </xsl:choose>
