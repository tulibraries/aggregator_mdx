...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="isPartOf">
        <xsl:if test="normalize-space(lower-case(.))">
            ...
            <xsl:if test="$setID = $setSpecList/padig:set or $setID = $setSpecList/padig:set">
                <xsl:element name="dcterms:isPartOf">
                    <xsl:value-of select="$setSpecList/padig:set[(. = $setID) or (. = $setID)]/@string"/>
                </xsl:element>
            </xsl:if>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="identifier">
        <xsl:if test="normalize-space(.) != '' and starts-with(.,'https://digitalcollections.statelibrary.pa.gov/')">
            <xsl:variable name="itemID" select="substring-after(.,'/detail/')"/>
            <xsl:variable name="baseURL" select="normalize-space('https://digitalcollections.statelibrary.pa.gov/')"/>
        ...
        <xsl:if test="normalize-space(.) != '' and contains(.,'/documents/')">
